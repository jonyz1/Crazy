...
const CrazyCardGame = () => {
  ...
  useEffect(() => {
    ...
    const wsConnect = (wsUrl) => {
      ...
      hello1.current=false
      socket.onopen = () => {
        console.log("Connected to WebSocket server.",hello1.current===true);
        if (hello1.current!=true){
        ...
        switch (message.value) {
          ...
          case "oppounts":
              setOpponents((prevOpponents) => {
                const existingIndex = prevOpponents.findIndex(o => o.which === message.which);

                if (existingIndex !== -1) {
                  // Update existing opponent's card count
                  const updatedOpponents = [...prevOpponents];
                  updatedOpponents[existingIndex] = message;
                  return updatedOpponents;
                } else {
                  // Add new opponent
                  return [...prevOpponents, message];
                }
              });
              break;
...
//             socket.send(JSON.stringify(moveMessage));
    ...
    const initialIp = sessionStorage.getItem("ip");
    if (!initialIp) {
  ...
  {opponents.map((opponent, index) => (
    <OpponentCards key={index} opponents={opponent} />
  ))}
</div>
